...
// Tambahkan import Dialog di bagian atas bersama import UI lainnya
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import { SelectGroup, SelectLabel, SelectSeparator } from "@/components/ui/select"; // Update import Select
import { Save, Trash2 } from "lucide-react"; // Pastikan icon Save diimport
...
// ... state yang sudah ada sebelumnya ...
  ...
  // --- TAMBAHAN BARU: STATE UNTUK CUSTOM PRESETS ---
  // State untuk menyimpan list preset buatan user
  const [customPresets, setCustomPresets] = useState<BeamConfig[]>([]);
  // State untuk nama preset baru saat mau disimpan
  const [newPresetName, setNewPresetName] = useState("");
  // State untuk membuka/tutup dialog simpan
  const [isSaveDialogOpen, setIsSaveDialogOpen] = useState(false);
  // State untuk mengetahui ID preset yang sedang dipilih (untuk tombol delete)
  const [selectedPresetId, setSelectedPresetId] = useState<string>("4.3-1");

  // Effect: Load custom presets dari LocalStorage saat pertama kali buka
  useEffect(() => {
    const saved = localStorage.getItem("mekban-custom-presets");
    if (saved) {
      try {
        setCustomPresets(JSON.parse(saved));
      } catch (e) {
        console.error("Gagal load preset", e);
      }
    }
  }, []);

  // Effect: Update LocalStorage setiap kali customPresets berubah
  useEffect(() => {
    localStorage.setItem("mekban-custom-presets", JSON.stringify(customPresets));
  }, [customPresets]);

  // Handler: Ganti Preset (Dimodifikasi untuk support Custom & Default)
  const handlePresetChangeWrapper = (val: string) => {
    setSelectedPresetId(val);

    // Cek apakah ini preset bawaan (dari file constants)
    if (PRESETS[val]) {
      setConfig(PRESETS[val]);
      return;
    }

    // Cek apakah ini preset custom
    const foundCustom = customPresets.find((p) => p.id === val);
    if (foundCustom) {
      setConfig(foundCustom);
      return;
    }

    // Jika pilih custom (kosong)
    if (val === "empty") {
       // Reset ke default kosong atau config saat ini
       // Opsional: setConfig ke default kosong jika diinginkan
    }
  };

  // Handler: Simpan Preset Baru
  const handleSavePreset = () => {
    if (!newPresetName.trim()) return alert("Nama preset tidak boleh kosong!");
    
    const newId = `custom-${Date.now()}`;
    const newPreset: BeamConfig = {
      ...config, // Ambil config saat ini
      id: newId, // Override ID dengan ID baru agar unik
      // Kita bisa simpan nama di property tambahan jika type BeamConfig mengizinkan,
      // atau kita simpan mapping nama di tempat lain. 
      // Asumsi: Kita inject property 'name' sementara ke object config 
      // (Typescript mungkin butuh 'as any' atau update interface BeamConfig)
      name: newPresetName 
    } as any; 

    const updatedList = [...customPresets, newPreset];
    setCustomPresets(updatedList);
    setSelectedPresetId(newId); // Otomatis pilih yang baru disimpan
    setNewPresetName(""); // Reset input nama
    setIsSaveDialogOpen(false); // Tutup dialog
  };

  // Handler: Hapus Preset
  const handleDeletePreset = () => {
    if (!selectedPresetId.startsWith("custom-")) return;
    
    if (confirm("Yakin ingin menghapus preset ini?")) {
      const updatedList = customPresets.filter(p => p.id !== selectedPresetId);
      setCustomPresets(updatedList);
      handlePresetChangeWrapper("empty"); // Kembali ke kosong setelah hapus
    }
  };
      ...
      >
        ...
        >
         {/* Preset Selector - Outside Trigger to avoid button nesting */}
          <div className="border-b px-4 py-3 sm:px-6 sm:py-3 bg-slate-50/50 dark:bg-slate-900/20">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              ...
              <div className="flex-1 space-y-2">
                ...
                <Select 
                  ...
                  onValueChange={handlePresetChangeWrapper}
                >
                  <SelectTrigger
                    id="preset-select"
                  ...
                  <SelectContent>
                    ...
                    {/* Group: Preset Bawaan */}
                    <SelectGroup>
                      <SelectLabel className="text-xs text-slate-400 mt-2">Soal Bawaan (Default)</SelectLabel>
                    ...
                    {/* Group: Preset Custom User */}
                    {customPresets.length > 0 && (
                      <>
                        <SelectSeparator />
                        <SelectGroup>
                          <SelectLabel className="text-xs text-blue-500 mt-2 font-bold">Kustom (Milikmu)</SelectLabel>
              ...
              {/* Bagian Tombol Action (Simpan & Hapus) */}
              <div className="flex items-center gap-2 pb-[1px]">
                {/* Tombol Hapus (Hanya muncul jika preset custom dipilih) */}
                ...
                {/* Tombol Simpan (Dialog) */}
                <Dialog open={isSaveDialogOpen} onOpenChange={setIsSaveDialogOpen}>
                  <DialogTrigger asChild>
                    <Button 
                      variant="outline" 
                      className="h-10 gap-2 border-blue-200 text-blue-600 hover:bg-blue-50 hover:text-blue-700 dark:border-blue-800 dark:text-blue-400 dark:hover:bg-blue-900/30"
                    >
                  ...
                  <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                      <DialogTitle>Simpan Konfigurasi Saat Ini</DialogTitle>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
                      <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <Input
                          ...
                          placeholder="Contoh: Latihan UTS 1"
                          className="col-span-3"
                        />
                      </div>
                    </div>
                    <DialogFooter>
                       <Button variant="ghost" onClick={() => setIsSaveDialogOpen(false)}>Batal</Button>
                      <Button onClick={handleSavePreset}>Simpan</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </div>
